...
class BestScores(QtGui.QMainWindow):
    ...
    def initUI(self):
        ...
        self.updateScoresOnline()
        self.readScores() 
        self.showScores()
        self.center()
    def updateScoresOnline(self):
        ftp = FTP('flapflap.cba.pl')
        ftp.login("admin@flapflap.cba.pl", "flap")
        ftp.cwd('FlappyScores')    
        for i in [0,1,3,5,7]:
            f = open("./resources/bestScores_" + str(i) + ".txt", "wb")
            ftp.retrbinary("RETR bestScores_" + str(i) + ".txt", f.write)
            f.close()
        ftp.quit()
    def readScores(self):
        """
    ...
    def updateScoresDiffChange(self):
        ...
        ftp.login("admin@flapflap.cba.pl", "flap")
        ftp.cwd('FlappyScores')  
        f = open("./resources/bestScores_" + str(self.parent.difficulty) + ".txt", "wb")
        ftp.retrbinary("RETR bestScores_" + str(self.parent.difficulty) + ".txt", f.write)
        f.close()
        ftp.quit()
        self.readScores()
        for i in range(0,5):
    ...
    def updateScores(self, name, score):
        ...
        for i in range(0,5):
            ...
            f.write(line)
        f.close()
        f = open("./resources/bestScores_" + str(self.parent.difficulty) + ".txt", "rb")
        ftp.storbinary("STOR bestScores_" + str(self.parent.difficulty) + ".txt", f)
        f.close()
        ftp.quit()
        self.readScores()
        for i in range(0,5):
